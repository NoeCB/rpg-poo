...
export default function TrialPage() {
  ...
  const handleSaveGame = async () => {
    try {
      ...
      if (res.ok) {
        toast.success('Progreso sellado por la Entidad. Abandonando la partida...');
        setTimeout(() => {
            ...
            <div className="flex-1 p-4 flex flex-col bg-zinc-950/80 font-[family-name:var(--font-special-elite)]">
              ...
              {!gameState.partidaTerminada && (
                <div className="flex-1 flex flex-col gap-2 justify-end">
                  ...
                  {/* Action Panel - Creepy corruped design */}
                  <div className="bg-stone-950 border-[3px] border-double border-red-950 p-4 rounded-none flex flex-col gap-2 shadow-[inset_0_0_20px_rgba(0,0,0,0.9),0_0_25px_rgba(100,20,20,0.15)] relative overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-stone-900/40 via-stone-950 to-stone-950">
                    <p className="text-sm font-normal text-red-500 tracking-[0.2em] text-center border-b border-red-950/50 pb-2 mb-1 uppercase font-[family-name:var(--font-horroroid)]">PANEL DE ACCIONES</p>
                    {activeMode === 'NONE' ? (
                      <>
                  ...
                  <button 
                    ...
                    className="mt-2 w-full bg-stone-950 border-2 border-stone-800 text-stone-500 hover:text-green-500 hover:border-green-900 hover:bg-green-950/20 font-normal py-4 rounded-none tracking-[0.2em] uppercase transition-all shadow-[inset_0_0_10px_rgba(0,0,0,0.8)] active:scale-95"
                  >
                    GUARDAR Y ABANDONAR
                  </button>
                </div>
